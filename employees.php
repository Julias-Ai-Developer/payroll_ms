        ...
        if (empty($employee_id) || empty($name) || empty($position) || $basic_salary <= 0) {
            ...
            if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
                ...
                if ($check_result->num_rows > 0) {
                    ...
                    // Create new employee
                    $sql = "INSERT INTO employees (employee_id, name, position, basic_salary, allowances) VALUES (?, ?, ?, ?, ?)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("sssdd", $employee_id, $name, $position, $basic_salary, $allowances);
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="number" id="basic_salary" name="basic_salary" value="<?php echo $basic_salary; ?>" min="0" step="0.01" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
                
                <div>
                    <label for="allowances" class="block text-gray-700 text-sm font-bold mb-2">Allowances</label>
                    <input type="number" id="allowances" name="allowances" value="<?php echo $allowances; ?>" min="0" step="0.01" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
            </div>
