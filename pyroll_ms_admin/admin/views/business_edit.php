...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($business_name) || empty($registration_number) || empty($business_type) || 
        ...
        // Check if registration number is used by another business
        $check_sql = "SELECT id FROM businesses WHERE registration_number = ? AND id != ?";
        if ($stmt = $conn->prepare($check_sql)) {
            $stmt->bind_param("si", $registration_number, $business_id);
            $stmt->execute();
            $stmt->store_result();
            ...
            if ($stmt->num_rows > 0) {
                ...
                // Update business
                $update_sql = "UPDATE businesses SET business_name = ?, registration_number = ?, 
                              business_type = ?, address = ?, phone = ?, email = ?, 
                              registration_date = ?, status = ? WHERE id = ?";
                
                if ($update_stmt = $conn->prepare($update_sql)) {
                    $update_stmt->bind_param("ssssssssi", $business_name, $registration_number, 
                                            $business_type, $address, $phone, $email, 
                                            $registration_date, $status, $business_id);
                    ...
                    if ($update_stmt->execute()) {
                        ...
                        $error = "Error updating business: " . $update_stmt->error;
                    }
            ...
            <div class="p-8">
                        ...
                        <div>
                            ...
                            <nav class="text-sm mt-2">
                                <ol class="flex list-none p-0">
                                    ...
                                    <li class="text-gray-600 font-medium">Edit Business</li>
                                </ol>
                            </nav>
                        </div>
                        <div class="flex items-center space-x-3">
                            ...
                            <a href="business_view.php?id=<?php echo $business_id; ?>" class="inline-flex items-center px-4 py-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-all duration-200">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600 mt-1">
                                <span class="inline-flex items-center">
                                    ...
                                    Registration #: <?php echo htmlspecialchars($business['registration_number']); ?>
                                </span>
                                <span class="mx-3">•</span>
                                <span class="inline-flex items-center">
                                    <i class="far fa-calendar-alt mr-2"></i>
                                    Registered: <?php echo date('M d, Y', strtotime($business['registration_date'])); ?>
                                </span>
                            </p>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                    <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
                        ...
                        <p class="text-blue-100 text-sm mt-1">Modify the business details below</p>
                    </div>
                    
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $business_id; ?>" method="post" class="p-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="date" 
                                           ...
                                           value="<?php echo htmlspecialchars($business['registration_date']); ?>"
                                           class="form-input block w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:outline-none transition-all"
                                           required>
                                </div>
                            </div>
                        ...
                        <div class="mb-6">
                            <label for="status" class="block text-sm font-semibold text-gray-700 mb-2">
                                Business Status <span class="text-red-500">*</span>
                            </label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                ...
                <div class="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <div class="flex items-start">
                        ...
                        <div>
                            <h6 class="font-semibold text-yellow-900 mb-1">Update Guidelines</h6>
                            <ul class="text-sm text-yellow-800 space-y-1">
                                <li>• Ensure all information is accurate before updating</li>
                                <li>• Changes to registration number should match official documents</li>
                                <li>• Changing status to "Inactive" may affect associated business owners and employees</li>
                            </ul>
                        </div>
                    </div>
                </div>
